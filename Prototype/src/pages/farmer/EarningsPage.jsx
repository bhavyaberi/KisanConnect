...
export default function EarningsPage() {
  ...
  const localizedHistory = farmerEarningsHistory.map((item) => ({
    ...item,
    month: t(item.month),
  }));
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm">
          <p className="text-xs sm:text-sm text-stone-500">{t("earnings.thisMonth")}</p>
          <p className="mt-1 sm:mt-2 text-xl sm:text-2xl font-bold text-stone-900">{"\u20b9"}18,400</p>
          <p className="mt-1 text-xs text-emerald-600">{t("earnings.thisMonthDelta")}</p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm">
          <p className="text-xs sm:text-sm text-stone-500">{t("earnings.last6Months")}</p>
          <p className="mt-1 sm:mt-2 text-xl sm:text-2xl font-bold text-stone-900">{"\u20b9"}88,700</p>
          <p className="mt-1 text-xs text-stone-400">{t("earnings.acrossMonths", { n: farmerEarningsHistory.length })}</p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm">
          <p className="text-xs sm:text-sm text-stone-500">{t("earnings.ordersFulfilled")}</p>
          <p className="mt-1 sm:mt-2 text-xl sm:text-2xl font-bold text-stone-900">12</p>
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm">
        ...
        <div className="mt-4 h-48 sm:h-56 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={localizedHistory} margin={{ left: -20, right: 10 }}>
