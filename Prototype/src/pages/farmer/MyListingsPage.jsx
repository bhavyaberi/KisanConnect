...
export default function MyListingsPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {showForm && (
        <form onSubmit={handleAddListing} className="rounded-2xl border border-stone-200 bg-white p-5">
          ...
          <label className="mt-4 flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-stone-300 px-4 py-3 text-sm text-stone-500 hover:border-emerald-400 hover:text-emerald-700">
            <Camera size={16} />
            {form.photo || t("listings.photoAttach")}
            <input
              type="file"
              accept="image/*"
              className="hidden"
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-5">
        <div className="divide-y divide-stone-100">
          {listings.map((l) => (
            <div key={l.id} className="flex items-center gap-4 py-3 first:pt-0">
              <div className="flex-1">
                <p className="text-sm font-medium text-stone-800">{t(`crop.${l.cropId.toLowerCase()}`)}</p>
                <p className="text-xs text-stone-500">
                  {l.qty.replace("kg", t("unit.kg"))} &middot; {l.price.replace("/kg", t("unit.perKg"))}
                </p>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-stone-200 px-2.5 py-1.5 text-xs font-medium text-stone-600 hover:bg-stone-50"
              >
